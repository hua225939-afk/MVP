...
动态联动新增独立的版本化平台记录，只保存不属于学生项目正文的跨角色
数据：

| 字段 | 说明 |
| --- | --- |
| `schemaVersion` | 当前平台本地记录版本 |
| `teacherFeedback` | 教师对演示学生的简短评语，学生与家长可见 |
| `studentAttention` | `needs_attention / doing_well / null` 简单关注标记 |

演示身份关系是只读配置，固定为一名当前学生、一名演示教师、一名关联
家长、一个演示班级、一个合作伙伴校区与一个总部身份。学习进度、
项目正文和平台记录继续分键保存，任何一类迁移或损坏都不能清空另外
两类数据。

能力摘要不保存“提高百分比”。`RoleDashboardService` 根据项目决定、
页面和组件、互动逻辑、测试与修复、反馈采纳、版本与发布表达等实际
行为生成证据描述。
